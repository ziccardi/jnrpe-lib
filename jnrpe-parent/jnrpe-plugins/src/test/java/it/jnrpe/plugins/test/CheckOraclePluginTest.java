...
public class CheckOraclePluginTest implements Constants
{
    ...
    public void setup() throws Exception
    {
        ...
        PluginDefinition checkFile = PluginRepositoryUtil
                .parseXmlPluginDefinition(cl, cl
                        .getResourceAsStream("check_oracle_plugin.xml"));
        ...
        cr.addCommandDefinition(
                new CommandDefinition("CHECK_ORACLE_ALIVE", "CHECK_ORACLE")
        ...
        cr.addCommandDefinition(
                new CommandDefinition("CHECK_TABLESPACE", "CHECK_ORACLE")
                    .addArgument(new CommandOption("username", "$ARG1$"))
                    .addArgument(new CommandOption("password", "$ARG2$"))
                    .addArgument(new CommandOption("db", "$ARG3$"))
                    .addArgument(new CommandOption("server", "$ARG4$"))
    ...
    public void checkIsAliveOk() throws JNRPEClientException
    {
        ...
        ReturnValue ret = client.sendCommand("CHECK_ORACLE_ALIVE", "scott", "tiger", "mockdb", "127.0.0.1");
    ...
    public void checkIsAliveKo() throws JNRPEClientException
    {
        JNRPEClient client = new JNRPEClient(BIND_ADDRESS, JNRPE_PORT, false);
        client.setTimeout(120);
        ReturnValue ret = client.sendCommand("CHECK_ORACLE_ALIVE", "scott", "tiger", "mockdbko", "127.0.0.1");
...
//                ./check_nrpe -t 20 -n -H myjnrpeserver -c check_oracle_ts -a 'username!password!SID!server!tablespacename'
        ...
        JNRPEClient client = new JNRPEClient(BIND_ADDRESS, JNRPE_PORT, false);
        client.setTimeout(120);
        ReturnValue ret = client.sendCommand("CHECK_TABLESPACE", "scott", "tiger", "mockdb", "127.0.0.1", "system");
        Assert.assertEquals(ret.getStatus(), Status.OK);
    }
...
//                ./check_nrpe -t 20 -n -H myjnrpeserver -c check_oracle_ts -a 'username!password!SID!server!tablespacename'
        ...
        JNRPEClient client = new JNRPEClient(BIND_ADDRESS, JNRPE_PORT, false);
        client.setTimeout(120);
        ReturnValue ret = client.sendCommand("CHECK_TABLESPACE", "scott", "tiger", "mockdb", "127.0.0.1", "user");
...
//                ./check_nrpe -t 20 -n -H myjnrpeserver -c check_oracle_ts -a 'username!password!SID!server!tablespacename'
        ...
        JNRPEClient client = new JNRPEClient(BIND_ADDRESS, JNRPE_PORT, false);
        client.setTimeout(120);
        ReturnValue ret = client.sendCommand("CHECK_TABLESPACE", "scott", "tiger", "mockdb", "127.0.0.1", "temp");
        Assert.assertEquals(ret.getStatus(), Status.CRITICAL);
    }
